...
class DrawingApp:
    def __init__(self, root):
        ...
        self.image = Image.new("RGBA", (500, 500), "white")  # 캔버스 크기 500x500으로 변경
        ...
        # 캔버스 생성 및 이미지 표시 설정
        self.canvas = tk.Canvas(self.root, width=500, height=500)  # 캔버스 크기 500x500으로 변경
        self.canvas.pack()
    ...
    def create_buttons(self):
        ...
        for i, (text, command) in enumerate(buttons):
            # `padx`, `pady`를 제거하여 버튼 사이의 공백을 없앰
            tk.Button(self.menu_frame, text=text, command=command).grid(row=0, column=i, sticky="ew")
    ...
    def add_text(self):
        ...
        if text:
            ...
            y = simpledialog.askinteger("Y 좌표", "텍스트의 Y 좌표 입력 (0~500):")  # 최대값을 500으로 수정
...
if __name__ == "__main__":
    root = tk.Tk()
    app = DrawingApp(root)
    root.geometry("600x700")  # 윈도우 크기 설정 (적절한 크기로 설정)
    root.mainloop()
