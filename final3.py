...
class DrawingApp:
    def __init__(self, root):
        ...
        self.image = Image.new("RGBA", (400, 400), "white")  # 이미지 크기를 400x400으로 수정
        ...
        # 캔버스 생성 및 이미지 표시 설정
        self.canvas = tk.Canvas(self.root, width=400, height=400)  # 캔버스 크기도 400x400으로 수정
        self.canvas.pack()
    ...
    def add_text(self):
        ...
        if text:
            x = simpledialog.askinteger("X 좌표", "텍스트의 X 좌표 입력 (0~400):")  # X 좌표 범위를 0~400으로 수정
            y = simpledialog.askinteger("Y 좌표", "텍스트의 Y 좌표 입력 (0~400):")  # Y 좌표 범위를 0~400으로 수정
            ...
            draw.text((x, y), text, fill=self.text_color, font=None)  # 폰트 크기 설정하려면 추가 필요
