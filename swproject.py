import tkinter as tk # Tkinter 라이브러리: GUI를 생성하기 위한 표준 라이브러리
from tkinter import colorchooser, filedialog, simpledialog # 색상 선택, 파일 대화상자, 간단한 대화상자 관련 기능
...
<<<<<<< HEAD
        if self.shape == "pen": # 펜 모드에서만 그리기 작업을 수행
            ...
            if self.pen_type in ["기본", "두껍게"]: # 기본 또는 두껍게 펜 타입일 때
                self.draw.line([self.last_x, self.last_y, event.x, event.y], fill=self.pen_color, width=pen_width) # 선 그리기
            elif self.pen_type == "가늘게": # 가늘게 펜 타입일 때
...
<<<<<<< HEAD
        ...
        if text:
            ...
            y = simpledialog.askinteger("Y 좌표", "텍스트의 Y 좌표 입력 (0~700):") # Y 좌표 입력 받기
            font_size = self.font_size_scale.get() # 텍스트 크기 가져오기
            self.canvas.create_text(x, y, text=text, fill=self.text_color, font=("Arial", font_size)) # 캔버스에 텍스트 추가

    def clear_canvas(self):
        # 전체 지우기 (캔버스를 초기화, 흰색 배경으로 설정)
        self.image = Image.new("RGBA", (500, 700), "white") # 새 이미지 객체 생성
        self.draw = ImageDraw.Draw(self.image) # 이미지에 그리기 위한 객체 생성
        self.update_canvas_image() # 캔버스에 이미지 업데이트
